\chapter{Améliorations - Code final}
\begin{figure}[!ht]
    \centering
...
class Tentacule_clothoïdaire(_Tentacule):
    ...
    def __init__(self, voiture, courbure_init, var_courbure, axes):
        ...
        self.patch = matplotlib.lines.Line2D(self.xdata, self.ydata, color='pink')
...
class Tentacule_Circulaire(_Tentacule):
    ...
    def __init__(self, voiture, k, delta_k, axes):
        ...
        self.patch = matplotlib.patches.Arc((centre_x, self.voiture.y), self.rayon, self.rayon, theta1=self.angle_debut, theta2=self.angle_fin, color='pink')
        # On applique la rotation de la voiture au dessin de la tentacule
        self.patch.set_transform(matplotlib.transforms.Affine2D().rotate_deg_around( self.voiture.x, self.voiture.y, self.voiture.angle - 90) + self.axes.transData)
...
class Voiture:
    ...
    def __init__(self, shape_voiture, axes, type_tentac=Tentacule_Circulaire, nombre_tentac=80):
        ...
        print(self.shapely_poly, self.shape_voiture)
        ...
        self.axes_vitesse.set_ylim(0, 1.3 * self.vitesse_max)
    ...
    def get_tentacules_navigables_et_lisses(self, objets_alentours):
        """
        Permet d'obtenir toutes les tentacules navigables et 'lisses' ie dont la variation de braquage avec le braquage courant demandée n'est pas trop forte (<CONE_BRAQUAGE).
